...
const showProducts = (products) => {
  ...
  for (const product of allProducts) {
    ...
    div.innerHTML = `<div class="single-product  mb-5 py-3 mx-2" style="height:380px;background-color:
    #dedce0
    ">
      <div>
      <div class="img-fluid">
      ...
      <h3 style="color:#bd0817">${product.title}</h3>
      <h5 class="fs-4 text-success">Category: ${product.category}</h5>
      <h2>Price: $ ${product.price}</h2>
      <h3 class="text-danger">Rating: ${product.rating.rate}</h3>
      <h4 class="text-dark">Reviews: ${product.rating.count}</h4>
      <div>
      <button onclick="addToCart(${product.id},${product.price})" id="addToCart-btn" class="buy-now btn btn-danger fs-4">Add to Cart</button>
      <button id="details-btn" class="btn btn-dark fs-4">Details</button>
      </div>
      </div>
      `;
